...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "sparseMatrixUtils", "tests")); system.time(test_check("sparseMatrixUtils", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran sparseMatrixUtils_$(VERSION).tar.gz
	rm -rf sparseMatrixUtils.Rcheck/
